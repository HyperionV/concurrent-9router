...
export async function handleForcedSSEToJson({
  ...
  if (isCodexResponsesApi) {
    try {
      if (dispatcherHooks?.onStreamStarted) {
        await dispatcherHooks.onStreamStarted();
      }
      const jsonResponse = await convertResponsesStreamToJson(
        providerResponse.body,
      );
      if (
        dispatcherHooks?.onResponseIdentity &&
        typeof jsonResponse?.id === "string" &&
        jsonResponse.id.trim() !== ""
      ) {
        await dispatcherHooks.onResponseIdentity(jsonResponse.id);
      }
  ...
  try {
    ...
    if (
      ...
      await dispatcherHooks.onResponseIdentity(parsed.id);
    }
