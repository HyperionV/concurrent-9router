...
function getGroupKey(item, keyField) {
  switch (keyField) {
    ...
    case "keyName":
      return item.apiKeyKey || item.keyName || "Unknown Key";
    case "endpoint":
...
export default function UsageStats() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/usage/stats?${usageQuery}`, { signal: controller.signal })
      .then(async (r) => {
        const data = await r.json().catch(() => null);
        if (!r.ok) throw new Error(data?.error || "Failed to fetch usage stats");
        return data;
      })
      .then((data) => {
      ...
      .catch((error) => {
        if (error?.name !== "AbortError") {
          ...
          setStatsError(error.message || "Failed to fetch usage stats");
        }
      })
      .finally(() => {
  ...
  }, [usageQuery]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 self-end">
        {period === "custom" && (
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-bg-subtle p-1.5 text-xs text-text-muted">
      ...
      {loading ? spinner : <UsageChart query={usageQuery} />}
